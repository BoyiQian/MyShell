...
{
       0,    59,    59,    62,    70,    74,    74,    85,    86,    90,
      95,   102,   108,   113,   117,   124,   125,   129,   133,   137,
     142,   143,   147,   148
};
#endif
...
#line 62 "shell.y" /* yacc.c:1646  */
    ...
    if (!hasWild((char*)(yyvsp[0].cpp_string)->c_str())) {
...
#line 90 "shell.y" /* yacc.c:1646  */
    ...
    Shell::_currentCommand._appends = true;
  }
...
int compare(const void* file1, const void *file2) {
  const char *a = *(const char **)file1;
  const char *b = *(const char **)file2;
  return strcmp(a,b);
}
...
void expandWildcardsIfNecessary(char * arg) {
  ...
  if(!strchr(arg, '*') && !strchr(arg, '?')) {
	...
	sortArrayStrings(array,nEntries);                                                                                                                                                                               for (int i = 0; i < nEntries;i++) {
  ...
  *r = '$'; r++; *r = '\0';
  ...
  while ((ent = readdir(dir)) != NULL) {
	if(regexec(&re, ent->d_name, 1, &match ,0) == 0) {
		char* value = strdup(ent->d_name);
		/*Command::_currentSimpleCommand->insertArgument(new std::string(value));*/
...
void expandWildcard(char *prefix, char *suffix) {
   ...
   if (prefix == NULL && suffix[0] == '/' && suffix[1] == 'd') {
	prefix = strdup("/");
	check = 1;
	
   }
   /*if (suffix[0] == 0 && !subdir) {
   ...
   /*printf("SSSSsuffix: %s",suffix);
   printf("CCC: %s", component);*/
		...
		if (suffix[0] != NULL && check) {
			...
			if(ent->d_type == DT_DIR) {
				if( strcmp(dir_name,"/")) {
					...
					sprintf(newPrefix, "%s/%s", dir_name, ent->d_name);
				/*printf("last: %s\n", newPrefix);*/
				}
				expandWildcard(newPrefix, suffix);
			}
			...
			if(prefix) {
				...
				if (argument[0] != '\0') {
					array[nEntries] = strdup(argument);
				} else {
					array[nEntries] = strdup(ent->d_name);
				}
				nEntries++;
